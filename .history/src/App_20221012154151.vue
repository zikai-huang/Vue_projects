  ...
  <div id="app">
    ...
    <ul aria-labelledby="list-summary" class="stack-large" id="todo">
      <li v-for="item in show" :key="item.id">
        <todo-item :label="item.label" :done="item.done" :id="item.id" @click-item="click_item(item.id)"></todo-item>
      </li>
    </ul>
    <span class="items-count">
      <check-all @show-all="show_all" @del-all="del_all" class="show-all" ></check-all>
...
export default {
  ...
  methods: {
    ...
    del_all() {
      this.TodoItems.splice(0, this.TodoItems.len);
    },
    click_item(id) {
