  ...
  <div id="app">
    ...
    <ul aria-labelledby="list-summary" class="stack-large" id="todo">
      <li v-for="item in show" :key="item.id">
        <todo-item v-bind="item" @click-item="click_item(item.id)"></todo-item>
      </li>
    </ul>
    <check-all @show-all="show_all" @delete-all="delete_all"></check-all>
    <span class="items_count">
      Undo:{{TodoItems.length-TodoItems.filter(item=>item.done).length}} &emsp; Done:{{TodoItems.filter(item=>item.done).length}} &emsp; All:{{TodoItems.length}}
    </span>
