  ...
  <div id="app">
    ...
    <ul aria-labelledby="list-summary" class="stack-large" id="todo">
      <li v-for="item in show" :key="item.id">
        <todo-item v-bind="item" @checkbox-changed="updateDoneStatus(item.id)" @item-edited="itemEdit" @item-deleteed="deleteItem"></todo-item>
      </li>
    </ul>
...
export default {
  ...
  methods: {
    ...
    itemEdit(id, newlabel) {
      const item = this.TodoItems.findIndex((item) => item.id === id);
